...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels, extra_blocks=None):
        ...
        for in_channels in in_channels_list:
            ...
            self.dppm = DPPM(in_channels, out_channels)
            self.cam = CAM(inplanes=out_channels)
    ...
    def forward(self, x: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        for idx in range(len(x) - 2, -1, -1):
            ...
            inner_top_down = self.dppm(inner_top_down)
            last_inner = inner_lateral + inner_top_down + self.cam(inner_top_down)
        ...
        outs = OrderedDict([(k, v) for k, v in zip(names, results)])
